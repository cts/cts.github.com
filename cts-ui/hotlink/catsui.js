...
(function() {
  ...
  __t('CATSUI').Clipboard = (function() {
    ...
    function Clipboard(clipboardServer) {
      ...
      var _this = this;
      this.clipboardServer = clipboardServer || "http://people.csail.mit.edu/karger/Cookie/cookie-server.html";
      this.clipboardKey = "cats-clipboard";
      this.loading = $.Deferred();
      this.serverWindow = null;
      window.addEventListener("message", this.onLoad, false);
      $(function() {
        _this.frame = $("<iframe src='" + _this.clipboardServer + "'></iframe>");
        _this.frame.hide();
        return $('body').append(_this.frame);
      });
    }

    Clipboard.prototype.onLoad = function(event) {
      if (event.source !== this.frame.get(0).contentWindow) {
        return;
      }
      window.removeEventListener("message", this.onLoad);
      this.serverWindow = event.source;
      return this.loading.resolve();
    };

    Clipboard.prototype.copy = function(text) {
      this.loading.done(function() {});
      return this.serverWindow.postMessage({
        cmd: "set",
        name: this.clipboardKey,
        value: text,
        days: 7
      }, "*");
    };
    ...
    Clipboard.prototype.paste = function(callback) {
      ...
      this.loading.done(function() {
        ...
        returnData = function(event) {
          if (event.source === _this.serverWindow) {
        ...
        return window.addEventListener("message", returnData, false);
      });
      return this.serverWindow.postMessage({
        cmd: "get",
        name: this.clipboardKey
  ...
  __t('CATSUI').ElementPicker = (function() {
    ...
    ElementPicker.prototype.enablePicker = function(callback) {
      if (this.state === 'OFF') {
        ...
        $('html').bind('keyup', this.keyUp);
        $('html').bind('mousemove', this.mouseMove);
        return $('html').bind('mouseup', this.mouseUp);
      }
    };
    ...
    ElementPicker.prototype.disablePicker = function() {
      if (this.state === 'ON') {
        ...
        $('html').unbind('keyup', this.keyUp);
        $('html').unbind('mousemove', this.mouseMove);
        return $('html').unbind('mouseup', this.mouseUp);
      }
    };
  ...
  __t('CATSUI').Modal = (function() {
    ...
    function Modal(title, content, ok, cancel, okCallback, cancelCallback) {
      ...
      this.cancalCallback = cancelCallback;
    }

    Modal.prototype.show = function() {
      var modal;
      modal = " <div class=\"modal webCatsContainer\" id=\"catsuiModal\">\n   <div class=\"modal-header\">\n     <button class=\"close\" data-dismiss=\"modal\">×</button>\n     <h3>" + this.title + "</h3>\n   </div>\n   <div class=\"modal-body\">" + this.content + "</div>\n   <div class=\"modal-footer\">\n     <a href=\"#\" class=\"btn\">" + this.cancel + "</a>\n     <a href=\"#\" class=\"btn btn-primary\">" + this.ok + "</a>\n   </div>\n</div>";
      this.modal = $(modal);
      this.modal.hide();
  ...
  __t('CATSUI').UI = (function() {
    ...
    function UI() {
      ...
      this.pasteTargetSelected = __bind(this.pasteTargetSelected, this);
    ...
    UI.prototype.copyTargetSelected = function(target) {
      var html, wrapper;
      wrapper = $("<div />");
    ...
    UI.prototype.pasteTargetSelected = function(target) {
      ...
      $(".pasteCancelBtn").click(function(event) {
        ...
        _this.callout.close(_this.target);
        return _this.target = null;
      });
      return $(".pasterBtn").click(function(event) {
        ...
        _this.callout.close(_this.target);
        return _this.clipboard.paste(function(content) {
          _this.performPaste(_this.target, opts, content);
          return _this.target = null;
        });
      });
    };

    UI.prototype.performPaste = function(anchor, options, html) {
      ...
      if (opts.data && opts.style) {
        ...
        data = this.engine.extractData(node);
        console.log("Pasted data:");
        console.log(data);
        template = this.engine.extractTemplate(anchor);
        replacementNode = this.engine.render(template, [data]);
        replacementNode = template;
      } else if (opts.style && !opts.data) {
        data = this.engine.extractData(anchor);
        console.log("Pasted data:");
        console.log(data);
        template = this.engine.extractTemplate(node);
    ...
    UI.prototype.pasteNode = function(anchor, node, location) {
      switch (location) {
        case "replace":
  ...
  __t('CATSUI').Sidebar = (function() {
    ...
    Sidebar.prototype.buildUI = function() {
      ...
      this.copyButton = $("#catsui-btn-copy");
      this.pasteButton = $("#catsui-btn-paste");
      this.toggleBtn.click(function() {
        return _this.toggle();
      });
      this.copyButton.click(function() {
        _this.toggle();
        return _this.ui.copy();
      });
      return this.pasteButton.click(function() {
        _this.toggle();
        return _this.ui.paste();
      });
    };

    Sidebar.prototype.toggle = function() {
      if (this.container.css("left") === "0px") {
        return this.container.css("left", "-200px");
      } else {
        return this.container.css("left", "0px");
      }
    };
  ...
  __t('CATSUI').Util = (function() {
    ...
    Util.includeCSS = function(url) {
      var elem;
      elem = "<link rel='stylesheet' href='" + url + "' type='text/css' />";
      return $("head").append(elem);
    };
